...
{
    return const_iterator{root, nullptr, size() + 1, size()};
}
...
template <typename T>
void BinarySearchTree<T>::const_iterator::advance(bool recur_up)
{
    if (last_seen)
    {
        if (last_seen->left)
    ...
    else    // first iteration of depth n of the tree
    {
        last_seen = current;
    }
    ++traversed;
